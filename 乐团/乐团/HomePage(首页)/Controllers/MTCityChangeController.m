...
#import "MTCitySearhResultController.h"
#import "UIView+Extension.h"
#import "MTCityGroupsModel.h"
#import "MJExtension.h"
#import "CommonDefine.h"
#import "Masonry.h"

@interface MTCityChangeController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>

@property (nonatomic, weak) UISearchBar *searchBar;
@property (nonatomic, weak) UITableView *tableView;
@property (nonatomic, weak) UIView *cover;
@property (nonatomic, weak) MTCitySearhResultController *searchResult;
...
@implementation MTCityChangeController



- (NSArray *)cityGroups{
    if(!_cityGroups){
        _cityGroups = [MTCityGroupsModel objectArrayWithFilename:@"cityGroups.plist"];
    }
    return _cityGroups;
}

- (MTCitySearhResultController *)searchResult{
    if(!_searchResult){
        MTCitySearhResultController *searchResult = [[MTCitySearhResultController alloc] init];
...
- (void)setupCityChange{
    ...
    [searchBar setTintColor:MTColor(32, 192, 180)];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    MTCityGroupsModel *cityModel = self.cityGroups[section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MTCityGroupsModel *model = self.cityGroups[indexPath.section];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    // > 发送通知
    [[NSNotificationCenter defaultCenter] postNotificationName:MTCityChangedNotification object:nil userInfo:@{ MTSelectedCity:model.cities[indexPath.row],MTSelectedRegion:@"全部地区",MTSelectedSubRegion:@"全部",MTNotificationExit:@"NO",MTNotificationInit:@"YES"}];
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section{
    MTCityGroupsModel *model = self.cityGroups[section];
    return model.title;
}
...
//    
//    for(MTCityGroupsModel *model in self.cityGroups){
