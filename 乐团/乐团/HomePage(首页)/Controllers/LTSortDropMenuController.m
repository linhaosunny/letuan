...
@interface LTSortDropMenuController ()
...
@implementation LTSortDropMenuController
...
- (void)viewDidLoad {
    ...
    // > 设置排序界面
    [self setupSortDropMenu:[LTSortsModel sorts]];
...
- (void)sortChanged:(NSNotification *) notification{
     if([notification.userInfo[LTNotificationExit] isEqualToString:@"NO"]){
         // > 谓词搜索
         NSPredicate *predicate = [NSPredicate predicateWithFormat:@"label contains %@ ",notification.userInfo[LTSortSelected]];
         
         self.results = [[LTSortsModel sorts] filteredArrayUsingPredicate:predicate];
        
         // > 设置初始排序选中的按钮
         LTSortsModel *model = [self.results firstObject];
...
- (void)sortClick:(UIButton *) button{
    ...
    if(![self.selectedButton isEqual:button]){
        ...
        // > 发送通知
        [[NSNotificationCenter defaultCenter] postNotificationName:LTSortChangedNotification object:nil userInfo:@{LTSortSelected:button.titleLabel.text,LTSortVaule:@(button.tag),LTNotificationExit:@"YES"}];
