...
@interface LTHistoryViewController () <LTDealCellDelegate>
...
- (void)loadDealsFromDataBase{
    ...
    // > 新增数据
    [self.deals addObjectsFromArray:[LTDealTool recentDealsWithNumofPage:LTDataBaseDealPerPageNum withPage:self.currentPage]];
...
- (void)deleteCollectView:(UIBarButtonItem *) item{
    ...
    for(LTDealModel *deal in self.deals){
        if(deal.isChecking){
            [LTDealTool removeRecentDeal:deal];
...
#warning Incomplete implementation, return the number of items
    ...
    // > 刷新控件隐藏
    self.collectionView.footerHidden = (self.deals.count == [LTDealTool recentDealsCount]);
    // > 显示是否有数据
