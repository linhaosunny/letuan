...
@interface LTMapViewController ()<MKMapViewDelegate,LTNavigationTopMenuDelegate,UIPopoverPresentationControllerDelegate>
    ...
    if([notification.userInfo[LTNotificationExit] isEqualToString:@"YES"]){
        if(![(NSString *)notification.userInfo[LTMainCagtegorySelected] containsString:@"全部"]){
            if([(NSString *)notification.userInfo[LTSubCagtegorySelected] containsString:@"全部"]){
                self.category = notification.userInfo[LTMainCagtegorySelected];
            }else{
                self.category = notification.userInfo[LTSubCagtegorySelected];
            }
        }else{
            self.category = nil;
        }
...
- (void)mapView:(MKMapView *)mapView regionDidChangeAnimated:(BOOL)animated{
    ...
    self.lastRequest = [[LTHttpTool sharedHttpTool] httpToolRequestWithURL:@"v1/deal/find_deals" params:params successWithResult:^(DPRequest *request,id result) {
        ...
        if(result[@"deals"]){
            ...
            for(LTDealModel *deal in deals){
                
                LTCategoryModel *category = [LTCategoryModel categoryWithDeal:deal];
                
                for(LTBusinessesModel *model in deal.businesses){
                    // > 添加大头针
                    LTAnnotationModel *annotaion = [[LTAnnotationModel alloc] init];
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation{
    if(![annotation isKindOfClass:[LTAnnotationModel class]]) return nil;
    ...
    // > 设置图片
    annotationView.image = [UIImage imageNamed:((LTAnnotationModel *)annotation).icon];
    annotationView.canShowCallout = YES;
    
    UIImageView *imageView = [[UIImageView alloc] init];
    imageView.size = CGSizeMake(100, 80);
    [imageView sd_setImageWithURL:[NSURL URLWithString:((LTAnnotationModel *)annotation).image_url ] placeholderImage:[UIImage imageNamed:@"placeholder_deal"]];
