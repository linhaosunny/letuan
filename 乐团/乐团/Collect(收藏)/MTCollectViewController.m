...
//    close.customView.width = MTNavigationRightBarButtonItemWidth;
    ...
    // > 监听通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(collectChanged:) name:MTCollectChangeNotification object:nil];
...
- (void)dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:MTCollectChangeNotification object:nil];
}

- (void)collectChanged:(NSNotification *) notification{
    
    if([notification.userInfo[MTCollectSelect] isEqualToString:@"YES"]){
        [self.deals addObject:notification.userInfo[MTCollectValue]];
    }else{
        [self.deals removeObject:notification.userInfo[MTCollectValue]];
    }
...
- (void)editCollectView:(UIBarButtonItem *) item{
    ...
    if([item.title isEqualToString:@"编辑"]){
        ...
        // > 让修改模型进入编辑状态
        for(MTDealModel *deal in self.deals){
            deal.editing = YES;
        }
...
- (void)loadDealsFromDataBase{
    ...
    // > 新增数据
    [self.deals addObjectsFromArray:[MTDealTool collectDealsWithNumofPage:MTDataBaseDealPerPageNum withPage:self.currentPage]];
