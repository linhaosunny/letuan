...
@interface LTDetailViewController ()<UIWebViewDelegate,LTDeitalViewDelegate>
@property (nonatomic, weak) MutableScrollView *scrollView;
@property (nonatomic, weak) LTDeitalView *topView;
...
- (void)viewDidLoad {
    ...
    [self.view setBackgroundColor:LTGlobalBlackgroudColor];
    // > 设置详情页
    ...
    // > 添加浏览记录
    if([LTDealTool isRecentDeal:self.deal]){
        // > 移除旧的记录
        [LTDealTool removeRecentDeal:self.deal];
    }
    // > 新增
    [LTDealTool addRecentDeal:self.deal];
}


- (void)setupDetailView{
    MutableScrollView *scrollView = [[MutableScrollView alloc] init];
    [scrollView setBackgroundColor:LTGlobalBlackgroudColor];
    ...
    LTDeitalView *topView = [[LTDeitalView alloc] init];
...
- (void)loadDetailSingleDeal{
    ...
    // > 发送网络请求
    [[LTHttpTool sharedHttpTool] httpToolRequestWithURL:@"v1/deal/get_single_deal" params:params successWithResult:^(DPRequest *request,id result) {
        LTDealModel *deal = [LTDealModel objectWithKeyValues:[result[@"deals"] firstObject]];
        ...
        // > 设置收藏
        [weakSelf.topView detailViewCollectButtonSetSelected:[LTDealTool isCollectDeal:weakSelf.deal]];
...
#pragma mark - <LTDeitalViewDelegate>
...
- (void)deitalView:(LTDeitalView *)view didClickCollectButton:(UIButton *)button{
    ...
    if(!button.isSelected){
        [LTDealTool addCollectDeal:self.deal];
        [MBProgressHUD showSuccess:@"收藏成功！" toView:self.view];
    }else{
        [LTDealTool removeCollectDeal:self.deal];
        [MBProgressHUD showSuccess:@"取消收藏成功！" toView:self.view];
    }
    
    // > 发通知
    [[NSNotificationCenter defaultCenter] postNotificationName:LTCollectChangeNotification object:nil userInfo:@{LTCollectSelect:[NSString stringWithFormat:@"%@",!button.isSelected?@"YES":@"NO"],LTCollectValue:self.deal}];
}
